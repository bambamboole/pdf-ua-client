...
export default function BlockPalette({ schema, onSelectPage, onExport, onInsertPreset, onLoadInvoice }: Props) {
    return (
        <div className="flex flex-col gap-3">
            <div>
                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Blocks</h2>
                <div className="space-y-1">
                    {listBlockTypes(schema).map((type) => <PaletteItem key={type} type={type} />)}
                </div>
            </div>
            <div className="space-y-1 border-t border-gray-200 pt-3">
                <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">Presets</h2>
                {presets.map((preset) => (
                    <button
                        key={preset.name}
                        type="button"
