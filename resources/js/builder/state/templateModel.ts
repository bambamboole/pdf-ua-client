import type { BlockType, DataMap, EditorBlock, EditorModel, EditorRow, Json, Template } from '../types';
...
export function fromTemplate(template: Template, data: DataMap = {}): EditorModel {
    return {
        ...
        rows: (template.rows ?? []).map((row): EditorRow => ({
            uid: uid(),
            gap: row.gap ?? null,
...
export function addBlock(model: EditorModel, type: BlockType | string, opts: { rowUid?: string | null; index?: number | null } = {}): EditorModel {
...
export function moveBlock(model: EditorModel, blockUid: string, toRowUid: string | null, toIndex: number | null): EditorModel {
...
function mapBlock(model: EditorModel, blockUid: string, fn: (b: EditorBlock) => EditorBlock): EditorModel {
    return {
        ...model,
        rows: model.rows.map((row) => ({ ...row, blocks: row.blocks.map((b) => (b.uid === blockUid ? fn(b) : b)) })),
    };
}
...
export function updateBlockId(model: EditorModel, blockUid: string, rawId: string): EditorModel {
    const taken = new Set(model.rows.flatMap((r) => r.blocks.filter((b) => b.uid !== blockUid).map((b) => b.id)));
...
export interface FoundBlock { row: EditorRow; block: EditorBlock; rowIndex: number; blockIndex: number; }
