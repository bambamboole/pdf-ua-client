...
import {
    ...
    addBlock, removeBlock, moveBlock, removeRow, moveRow,
    updateBlockConfig, updateBlockId, updateTemplateConfig, setRowWidths,
    insertRows,
...
export default function TemplateBuilder({ schema, initialTemplate, initialData, renderTemplate, onChange }: Props) {
    ...
    useEffect(() => {
        ...
        debounceRef.current = setTimeout(() => {
            renderTemplate(toTemplate(model), toDataMap(model))
                .then((result) => { setHtml(result); setError(null); })
                .catch((cause: unknown) => setError(String((cause as Error)?.message ?? cause)));
        }, 300);
        return () => clearTimeout(debounceRef.current!);
    }, [model, renderTemplate, onChange]);

    const selectBlock = useCallback((uid: string) => { setSelectedBlockUid(uid); setPageSelected(false); }, []);
    const selectPage = useCallback(() => { setPageSelected(true); setSelectedBlockUid(null); }, []);
    ...
    const format = (model.config as { page?: { format?: string } })?.page?.format ?? getPageFormat(schema);
            ...
            <div className="flex h-screen">
                ...
                <main className="flex flex-1 flex-col overflow-hidden">
                    <div className="flex gap-1 border-b border-gray-200 bg-white px-4 py-2">
                        <button type="button" onClick={() => setTab('edit')} className={`rounded px-3 py-1 text-sm ${tab === 'edit' ? 'bg-gray-800 text-white' : 'text-gray-600'}`}>Edit</button>
                        <button type="button" onClick={() => setTab('data')} className={`rounded px-3 py-1 text-sm ${tab === 'data' ? 'bg-gray-800 text-white' : 'text-gray-600'}`}>Data</button>
                        <button type="button" onClick={() => setTab('preview')} className={`rounded px-3 py-1 text-sm ${tab === 'preview' ? 'bg-gray-800 text-white' : 'text-gray-600'}`}>Preview</button>
                    </div>
                ...
                <aside className="w-80 shrink-0 overflow-y-auto border-l border-gray-200 bg-white p-4">
                    <Inspector
                        ...
                        onUpdateBlockConfig={(uid, config) => setModel((m) => updateBlockConfig(m, uid, config as Json))}
                        onUpdateTemplateConfig={(config) => setModel((m) => updateTemplateConfig(m, config as Json))}
                    />
                </aside>
            </div>
            <DragOverlay>
