...
function block(id: string, type: EditorBlock['type'], config: Record<string, unknown>, data: Record<string, unknown>): EditorBlock {
...
export const presets: Preset[] = [
    ...
    {
        name: 'Bill to',
        build: () => [row(block('bill-to', 'key-value', {}, {
            entries: [
    ...
    {
        name: 'Line items',
        build: () => [row(block('line-items', 'table', {}, {
    ...
    {
        name: 'Totals',
        build: () => [row(block('totals', 'key-value', { align: 'right' }, {
            entries: [
...
export function invoiceExample(): { template: Template; data: DataMap } {
    const rows: EditorRow[] = [
        row(
            ...
            block('invoice-meta', 'key-value', { align: 'right', width: '40%' }, {
                entries: [
        ...
        row(
            block('from', 'key-value', { width: '50%' }, { entries: [{ label: 'From', value: 'ACME GmbH, Main St 1' }] }),
            block('to', 'key-value', { width: '50%' }, { entries: [{ label: 'Bill to', value: 'Beta Ltd, 2nd Ave' }] }),
        ),
        row(block('rule', 'divider', {}, {})),
        row(block('items', 'table', {}, {
            headers: ['Description', 'Qty', 'Unit', 'Amount'],
            rows: [['Consulting', '10', '€100', '€1000'], ['License', '1', '€250', '€250']],
        })),
        row(block('totals', 'key-value', { align: 'right' }, {
            entries: [
                { label: 'Subtotal', value: '€1250' },
                { label: 'Tax (19%)', value: '€237.50' },
                { label: 'Total', value: '€1487.50' },
        ...
        row(block('footer', 'text', {}, { text: 'Payment due within 14 days. Thank you for your business.' })),
    ];
