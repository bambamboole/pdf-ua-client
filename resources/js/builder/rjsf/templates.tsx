...
import {
    type BaseInputTemplateProps, type FieldTemplateProps,
    type ObjectFieldTemplateProps, type WidgetProps, getInputProps,
} from '@rjsf/utils';

export function BaseInputTemplate(props: BaseInputTemplateProps) {
    const {
        id, value, type, placeholder, required, disabled, readonly, autofocus,
        onChange, onChangeOverride, onBlur, onFocus, schema, options,
    } = props;
    ...
    return (
        <input
            ...
            autoFocus={autofocus}
            {...inputProps}
            onChange={onChangeOverride || ((e: ChangeEvent<HTMLInputElement>) =>
...
export function FieldTemplate(props: FieldTemplateProps) {
    const { id, classNames, style, label, help, required, description, errors, children, hidden } = props;
    if (hidden) {
...
export function ObjectFieldTemplate(props: ObjectFieldTemplateProps) {
    ...
    return (
        <fieldset className="mb-3 rounded border border-gray-200 p-3">
            {title && <legend className="px-1 text-xs font-semibold uppercase tracking-wide text-gray-400">{title}</legend>}
            {description && <p className="mb-2 text-xs text-gray-500">{description}</p>}
            {properties.map((el) => <div key={el.name}>{el.content}</div>)}
        </fieldset>
    );
}

export function SelectWidget(props: WidgetProps) {
    const { id, options, value, required, disabled, readonly, multiple, placeholder, onChange, onBlur, onFocus } = props;
    ...
    return (
        ...
        >
            ...
            {enumOptions.map((opt, i) => <option key={i} value={String(opt.value)}>{opt.label}</option>)}
        </select>
    );
}
