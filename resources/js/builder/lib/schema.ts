...
export function getBlockSubschemas(schema: JsonSchema, type: string): { props: JsonSchema; config: JsonSchema } {
    const emptySchema = (): JsonSchema => ({ type: 'object', properties: {}, $defs: (schema as any)?.$defs });
...
export function getTemplateConfigSchema(schema: JsonSchema): JsonSchema {
    const templateConfig = (schema as any)?.$defs?.templateConfig ?? { type: 'object', properties: {} };
