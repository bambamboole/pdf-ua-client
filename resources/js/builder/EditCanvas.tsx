...
function BlockBox({ block, rowUid, selected, style: layoutStyle, onSelect, onRemove }: BlockBoxProps) {
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between gap-2">
                <span {...listeners} {...attributes} className="cursor-grab font-medium text-gray-800">⠿ {humanizeType(block.type)}</span>
                <button type="button" onClick={(e) => { e.stopPropagation(); onRemove(block.uid); }} className="text-gray-400 hover:text-red-600">✕</button>
            </div>
...
function Row({ row, selectedBlockUid, onSelectBlock, onRemoveBlock, onRemoveRow, onSetRowWidths }: RowProps) {
    const { attributes, listeners, setNodeRef, transform, transition } = useSortable({
        id: `row-${row.uid}`,
        data: { source: 'row', rowUid: row.uid },
    });
    const { setNodeRef: setDropRef } = useDroppable({ id: `rowdrop-${row.uid}`, data: { source: 'row', rowUid: row.uid } });
    ...
    return (
        <div ref={setNodeRef} style={style} className="rounded border border-gray-200 bg-gray-50 p-2">
            <div className="mb-1 flex items-center justify-between text-xs text-gray-400">
                <span {...listeners} {...attributes} className="cursor-grab">⠿ row</span>
                <span className="flex gap-2">
                    <button type="button" onClick={() => onRemoveRow(row.uid)} className="hover:text-red-600">remove row</button>
                </span>
            </div>
            <div ref={(node) => { setDropRef(node); containerRef.current = node; }} className="flex gap-2">
                <SortableContext items={row.blocks.map((b) => b.uid)} strategy={horizontalListSortingStrategy}>
                    {row.blocks.map((block, i) => (
                        <Fragment key={block.uid}>
                            <BlockBox
                                ...
                                style={widths ? { flexBasis: `${percents[i]}%`, flexGrow: 0, flexShrink: 0 } : undefined}
...
function NewRowZone() {
    ...
    return (
        <div ref={setNodeRef} className={`rounded border-2 border-dashed px-3 py-6 text-center text-sm ${isOver ? 'border-blue-400 text-blue-500' : 'border-gray-300 text-gray-400'}`}>
            Drop a block here to add a new row
        </div>
    );
}

export default function EditCanvas({ model, selectedBlockUid, onSelectBlock, onRemoveBlock, onRemoveRow, onSetRowWidths }: Props) {
    return (
        <div className="space-y-3">
            <SortableContext items={model.rows.map((r) => `row-${r.uid}`)} strategy={verticalListSortingStrategy}>
                {model.rows.map((row) => (
                    <Row
                        key={row.uid}
                        row={row}
