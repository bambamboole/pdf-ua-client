...
function Collapsible({ title, children, defaultOpen = false }: { title: string; children: ReactNode; defaultOpen?: boolean }) {
    ...
    return (
        <div className="rounded border border-gray-200">
            <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between px-3 py-2 text-sm font-medium text-gray-700">
                <span>{title}</span><span className="text-gray-400">{open ? '▾' : '▸'}</span>
            </button>
...
export default function Inspector({ schema, selection, onUpdateBlockId, onUpdateBlockConfig, onUpdateTemplateConfig }: Props) {
    if (!selection) {
    ...
    return (
        <div className="space-y-3">
            <div>
                ...
                />
                <p className="mt-1 text-xs text-gray-400">{humanizeType(block.type)} — data key for injection</p>
            </div>
